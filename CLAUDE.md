...
本文件是给 Claude Code 使用的项目维护规则。  
它不是给用户看的 README，也不是 `domain_agent.py` 的 system prompt。

本项目是一个基于 **DeepSeek API + Python + Claude Code** 的本地专业 Agent 项目，目前包含五部分能力：
...
- 不要提交到 Git
- 不要在终端输出真实 Key
- 不要在代码、README、CLAUDE.md 或日志中暴露真实 Key
- 可以检查变量是否存在，但不能输出变量值
...
`examples/vision_test/test3.jpg`
"examples/vision_test/test3.jpg"
E:\BIT\Agent-MVP\Claude code\examples\vision_test\test3.jpg
```
...
## 13. 给 Claude Code 的总体原则

在本项目中，Claude Code 应遵守：
